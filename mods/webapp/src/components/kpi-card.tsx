...
function KpiCardPeriodSelect({ value, options, onChange, ariaLabel }: KpiCardPeriodControl) {
  ...
  return (
    ...
    <div className="relative inline-flex shrink-0 items-center gap-1 rounded-full border border-border bg-surface px-2 py-1 focus-within:ring-2 focus-within:ring-border">
...
export function KpiCard({ label, value, subtext, trend, className, period }: KpiCardProps) {
  return (
    <div className={cn("flex flex-col gap-2 border border-border bg-surface p-5", className)}>
      {period ? (
        ...
        <div className="flex items-center justify-between gap-2">
          <p className="min-w-0 truncate text-[13px] font-medium text-fg-subtle">{label}</p>
          <KpiCardPeriodSelect {...period} />
        </div>
      ) : (
        <p className="text-[13px] font-medium text-fg-subtle">{label}</p>
      )}
      <p className="text-[28px] font-bold leading-tight text-fg">{value}</p>
      ...
      {(trend || subtext) && (
        <div className="mt-auto flex flex-col gap-1">
          {trend && (
            <p
              className={cn("text-xs font-normal", trend.positive ? "text-primary" : "text-danger")}
            >
              {trend.value}
            </p>
          )}
          ...
          {subtext && <p className="text-xs font-normal text-fg-subtle">{subtext}</p>}
        </div>
      )}
    </div>
  );
