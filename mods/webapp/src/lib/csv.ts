...
export function parseCsv(text: string): { rows: CsvRow[]; errors: string[] } {
  ...
  if (missing.length > 0) {
    return {
      rows: [],
      errors: [`Columnas requeridas faltantes: ${missing.join(", ")}. Se esperaban: loan_id, full_name, outstanding_balance`]
    };
  }
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!externalId) { errors.push(`Fila ${i + 1}: loan_id vacío`); continue; }
    if (!fullName) { errors.push(`Fila ${i + 1}: full_name vacío`); continue; }
    if (!rawBalance) { errors.push(`Fila ${i + 1}: outstanding_balance vacío`); continue; }

    const lastPaymentDateRaw = c("last_payment_date");
    ...
    rows.push({
      ...
      lastPaymentDate: lastPaymentDateRaw || undefined,
