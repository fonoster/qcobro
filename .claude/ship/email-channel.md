...
| 5   | Sync            | done   | `openspec archive` promoted delta into main specs: +9 reqs across account-contact-log, agent-templates, channel-dispatch (updated) + campaigns-engine, email-channel (created). Post-fixes: Resend inbound is metadata-only → hydrate body via getReceivedEmail (GET /emails/receiving/{id}); stripQuotedReply; EMAIL insight from thread; English insight prompt. 111 apiserver + 15 common tests, lint + typecheck green. |
| 6   | Archive         | done   | Archived as `2026-06-27-email-channel`; `openspec validate --all --strict` = 26 passed. Verified live round-trip (real Gmail reply captured, autopilot reply + PAYMENT_PROMISE objective).                                                                                                                                                                                                                                  |
