...
Defines how a gestión's structured AI analysis (the `ai*` fields — summary, sentiment, debt
reason, result, next step) is produced from its conversation transcript: the injectable LLM
generator, provider configuration in `qcobro.json`, channel gating (transcript-only),
generation timing with persistence, multilingual output, and the guardrails for sending
transcripts to an external model.
