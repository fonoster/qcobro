...
The deterministic judge for engine runs: a pure `evaluate(events, parameters)` function
in `@qcobro/common` that replays a flight-recorder stream against a fixed invariant catalog
(safety: windows, attempt caps, suppression, rate caps, at-most-once; performance: tick
duration, dispatch latency, error rate, budget utilization; liveness: no starved accounts)
and produces a scorecard with per-invariant verdicts, per-campaign breakdowns, and
violations traceable to event ids. Consumed by the CI simulation harness and by the
`engine-eval` npx CLI against a live deployment.
