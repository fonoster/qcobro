...
The campaigns engine's flight recorder: an append-only, correlated stream of engine
events (tick lifecycle, campaign evaluations with config snapshots, per-account decisions,
dispatch attempts, inbound provider signals) persisted alongside the gestiones, exported
over an API-key-authenticated workspace-scoped endpoint, and pruned by a configured
retention window. Telemetry, not a system of record — gestiones remain the record of
contact attempts, and persistence is best-effort so it can never affect dispatch.
