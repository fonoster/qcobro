# sdk-agent-templates Specification

## Purpose

The `client.agentTemplates` resource of `@qcobro/sdk` — typed methods for listing,
reading, creating, and re-syncing agent templates, with client-side validation of inputs
against the shared `@qcobro/common` schemas. Mirrors `sdk-portfolios`'s pattern.
...
agent template by id, returning the base template row. The response does not include the
updated child config (e.g. `fonosterAppRef`); callers that need the resulting sync status
re-fetch it via `get`.

#### Scenario: Re-sync a previously unsynced template

- **WHEN** `sync({ id })` is called for a template whose prior Fonoster sync failed
- **THEN** the server re-attempts the sync and a subsequent `get({ id })` reflects the
  new outcome (`fonosterAppRef` populated on success)
