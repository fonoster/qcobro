...
- [x] 7.3 On an inbound customer message, record it on the gestión and trigger the smart agent: reuse the email agent's AI-reply mechanism (`systemPrompt`), send the reply via `WhatsAppClient.sendText`, cap at `maxReplies`, and refuse free-form replies once Meta's 24h window has closed
- [x] 7.4 Detect opt-out intent and payment-promise registration from the conversation (consistent with the email agent)
