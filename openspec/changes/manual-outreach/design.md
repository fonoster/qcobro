...
- **Voice (Fonoster).** Both Voz IA and pre-recorded place an outbound call to the
  ...
  - **Pre-recorded = EXTERNAL app:** all pre-recorded agents share ONE deployment-wide
    external app ref (`fonoster.prerecordedAppRef`) that points at the embedded
    **VoiceServer** hosted by the apiserver on its own port (`apiserver.voicePort`, default
    50061). Dispatch passes only the ready script as `metadata.message`; the VoiceServer
    answers, plays it with the Say verb, and hangs up (no templating at that level — the
    string arrives rendered).
