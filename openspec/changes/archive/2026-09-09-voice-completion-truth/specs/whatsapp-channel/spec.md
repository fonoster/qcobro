...
- `delivered` → `delivery` of `DELIVERED`
- `read` → `channelData.openedAt`, set once on the first read receipt
- `failed` → `delivery` of `FAILED`, with `deliveryReason` derived from Meta's error codes:
...
A `read` receipt SHALL NOT set `path` or `outcome`, and SHALL NOT by itself advance
`delivery`. Every status SHALL record its raw value in `channelData.deliveryStatus`, terminal or
not.
...
`channelData.optOutAt`, and SHALL additionally set the gestión's `outcome` to `OPT_OUT` when
it has no `outcome` yet. Both conditions are required: `131050` on a non-`failed` status
SHALL NOT be treated as an opt-out, since the message in fact reached the recipient.

`outcome` is single-valued, so an opt-out SHALL NOT overwrite a richer outcome the
...
#### Scenario: Delivered status advances delivery
...
- **AND** `delivery`, `path`, and `outcome` are unchanged
...
- **WHEN** a verified `failed` status carrying error code `131050` correlates to a gestión still
  at `DISPATCHED`
- **THEN** that gestión's `delivery` becomes `FAILED` with `deliveryReason` of `REJECTED`
- **AND** its `outcome` is set to `OPT_OUT`
...
- **WHEN** a verified `failed` status carrying `131050` correlates to a gestión whose
  `outcome` is already `PAYMENT_PROMISE`
...
- **WHEN** a `delivered` status correlates to a gestión whose `delivery` is already `FAILED`
