...
<ParamField body="outcome" type="string" required>
  Resultado de la gestión. Uno de `DISPATCHED`, `DELIVERED`, `NOT_DELIVERED`, `NO_ANSWER`,
  ...
  `REFUSED`. Usa `DISPATCHED` solo para dejar constancia de que el contacto se envió y el
  resultado todavía no se conoce: no cuenta como una gestión exitosa hasta que se reporte con
