...
Una gestión describe hasta tres cosas independientes: si el mensaje llegó (`delivery`), qué
camino tomó la interacción (`path`) y qué se obtuvo de ella (`outcome`). Son ejes
...
<ParamField body="delivery" type="string" default="DISPATCHED">
...
<ParamField body="deliveryReason" type="string">
  Por qué falló la entrega. Obligatorio cuando `delivery` es `FAILED`, y no se admite en
...
<Note>
  `SMS` y `VOICE_PRERECORDED` no tienen canal de vuelta, así que solo admiten `delivery`.
  Enviar `path` o `outcome` en esos canales se rechaza con `400`.
</Note>
