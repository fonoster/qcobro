...
<Steps>
  ...
  <Step title="Envía las filas">
    Pasa el `portfolioId`, un `mode` y al menos una fila (solo `REPLACE` acepta un lote
    vacío). Cada fila necesita un `externalId`,
...
<Tabs>
  ...
  <Tab title="REPLACE">
    ...
    cartera. Un lote vacío (`rows: []`) es una instantánea válida: vacía la cartera archivando
    todas sus cuentas.
...
cualquier petición. Un lote vacío en `APPEND_ONLY` o `UPDATE_EXISTING`, un `mode` desconocido
o un `email` malformado lanza un `ValidationError` con detalle por campo y no hace ninguna
llamada de red.
...
try {
  await client.portfolios.syncAccounts({
    portfolioId: "f47ac10b-...",
    mode: "APPEND_ONLY",
    rows: [] // los lotes vacíos se rechazan fuera de REPLACE
  });
} catch (err) {
